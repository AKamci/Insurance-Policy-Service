package PolicyProject.policyService.infrastructure.strategy.WeightStrategy.WeightStrategy.CarPriceStrategy;

import PolicyProject.policyService.infrastructure.persistence.entity.Weights;
import PolicyProject.policyService.infrastructure.strategy.WeightStrategy.IWeightStrategy.IWeightStrategy;

public class CarPrice_0_500K_Strategy implements IWeightStrategy {
    @Override
    public Weights getWeights(String key) {
        return null;
    }
}
